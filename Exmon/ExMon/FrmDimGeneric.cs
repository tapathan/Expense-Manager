...
namespace ExMon
{
    public partial class FrmDimGeneric : Form
    {
        string TableName;
        string KeyName;
        Font AllFont;
        ...
        Boolean ReadOnly;
        public FrmDimGeneric(string TableName)
        {
        ...
        public FrmDimGeneric(string TableName, string FormTitle)
        {
            this.TableName = TableName;
            this.KeyName = TableName.Replace("Dim", "Key");
            this.AllFont = new Font("Verdana", 8, FontStyle.Regular);
        ...
        public FrmDimGeneric(string TableName, string FormTitle, decimal DivideGridCellSizeBy)
        {
            this.TableName = TableName;
            this.KeyName = TableName.Replace("Dim", "Key");
            this.AllFont = new Font("Verdana", 8, FontStyle.Regular);
        ...
        public FrmDimGeneric(string TableName, string Fnt, string FormTitle, decimal DivideGridCellSizeBy)
        {
            this.TableName = TableName;
            this.KeyName = TableName.Replace("Dim", "Key");
        ...
        public FrmDimGeneric(string TableName, string Fnt, string FormTitle, decimal DivideGridCellSizeBy, Boolean ReadOnly)
        {
            this.TableName = TableName;
            this.KeyName = TableName.Replace("Dim", "Key");
            this.AllFont = new Font(Fnt, 8, FontStyle.Regular);
        ...
        public int XInitialLocation = 110, YInitialLocation = 380;
        ...
        private void FrmDimGeneric_Load(object sender, EventArgs e)
        {
            int ObjNumber = 0, Counter = 0;
            string[] AliasArr = new string[50];
            if (con.State == ConnectionState.Closed)
                con.Open();
            DtpCurrDate.Font = this.AllFont;
            TransFlex.Font = this.AllFont;
            LblDt.Font = this.LblFont;
        ...
        private void Create_TextBoxAndDtp(int Number)
        {
            ...
            while (Counter < Number)
            {
                ...
                if (Counter != StartDtPos && Counter != EndDatePos)
                {
                    ...
                    if (Counter < 10)
                        IncrementLocation = YInitialLocation + Counter * (Height - HeightDecrement) + 6;
                    else
                        IncrementLocation = YInitialLocation + Counter % (FieldColumns * 10) * (Height - HeightDecrement) + 6;
        ...
        private void Resize_grid(decimal DivideBy)
        {
            ...
            while (Counter < FieldsAlias.Length)
            {
                ...
                else
                {
                    ...
                    if (Convert.ToInt16(FieldsSize[Counter]) >= 35)
                        FieldsSize[Counter] = "25";
                    TransFlex.Columns[Counter].Width = Convert.ToInt16(10 * Convert.ToDecimal(FieldsSize[Counter]) / DivideBy - 6);
                }
                Counter++;
            }
        }
        ...
        private void Fill_grid(int Load, string QueryWhereClause)
        {
            ...
            strQ = "select " + SelectFields + " from " + TableName + " where EndDate >= '"
                + DtpCurrDate.Value.ToString("yyyy/MM/dd") + "' " + QueryWhereClause + " order by StartDate";
            ...
            while (Counter < NumberOfObject)
            {
                if (Counter != StartDtPos && Counter != EndDatePos)
                {
                    ...
                    if (Counter == StartDtPos)
                        DatePkArr[Counter].Value = DtpCurrDate.Value;
                    else
                        DatePkArr[Counter].Value = new DateTime(2099, 12, 31);
                Counter++;
            }
        }

        private void TransFlex_CellClick_1(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                int Counter = 0;
        ...
        private void DtpCurrDate_ValueChanged_1(object sender, EventArgs e)
        {
            Fill_grid(0, "");
        }
        ...
        public void Control_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                if (Counter != StartDtPos && Counter != EndDatePos && ImportantFieldInd[Counter] == 'Y')
                {
        ...
        {
            int StartDateKey = mgr.FetchKey("DateDim", DatePkArr[StartDtPos].Value.ToString("dd-MM-yyyy"));
            int EndDateKey;
            if (DatePkArr[EndDatePos].Value.ToString("dd-MM-yyyy") != "31-12-2099")
                EndDateKey = mgr.FetchKey("DateDim", DatePkArr[EndDatePos].Value.ToString("dd-MM-yyyy"));
            else
                EndDateKey = 0;
            if (con.State == ConnectionState.Closed)
                con.Open();
            ...
            //Update next recent record with the StartDate as the StartDate of the deleted record.
            int KeyFieldValue = 0;
            ...
            int NextKeyFieldValue = 0;
            strQ = "select isnull(min(" + KeyName + "),0) MKV from " + TableName + " where " + PrimaryFieldsWhereClauseGlobal
                + " and " + KeyName + " > " + KeyFieldValue;
            cmd = new SqlCommand(strQ, con);
            Rdr = cmd.ExecuteReader();
            //Rdr.Read();
            if (Rdr.HasRows == true)
            {
                Rdr.Read();
                NextKeyFieldValue = Convert.ToInt16(Rdr["MKV"]);
            }
            Rdr.Close();
            string StartDateValue = GetFieldValues(StartDtPos);
            //Get New Key Value
            int NewKeyValue = 0;
            strQ = "select " + KeyName + " NK from " + TableName
                + " where " + KeyName + " = " + NextKeyFieldValue;
            cmd = new SqlCommand(strQ, con);
            Rdr = cmd.ExecuteReader();
            if (Rdr.HasRows == true)
            {
                Rdr.Read();
                NewKeyValue = Convert.ToInt16(Rdr["NK"]);
            }
            Rdr.Close();

            strQ = "update " + TableName + " set StartDate = '" + StartDateValue
                + "' where " + KeyName + " = " + NextKeyFieldValue;
            cmd = new SqlCommand(strQ, con);
            cmd.ExecuteNonQuery();
            if (NextKeyFieldValue == 0)
            {
                strQ = "select isnull(max(" + KeyName + "),0) MKV from " + TableName + " where " + PrimaryFieldsWhereClauseGlobal
                + " and " + KeyName + " < " + KeyFieldValue;
                cmd = new SqlCommand(strQ, con);
                Rdr = cmd.ExecuteReader();
                //Rdr.Read();
                ...
                Rdr.Close();
                string DefaultEndDateValue = "2099/12/31";
                //Get New Key Value
                strQ = "select " + KeyName + " NK from " + TableName
                    + " where " + KeyName + " = " + NextKeyFieldValue;
                cmd = new SqlCommand(strQ, con);
                ...
                strQ = "update " + TableName + " set EndDate = '" + DefaultEndDateValue
                    + "' where " + KeyName + " = " + NextKeyFieldValue;
                cmd = new SqlCommand(strQ, con);
                cmd.ExecuteNonQuery();
        ...
        private void BtnUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                DateTime SelectedStartDateValue = Convert.ToDateTime(GetFieldValues(StartDtPos)).Date;
                DateTime SelectedEndDateValue = Convert.ToDateTime(GetFieldValues(EndDatePos)).Date;
                DateTime EndDt = new DateTime(2099, 12, 31);
                if (SelectedStartDateValue >= DtpCurrDate.Value.Date)
                {
                    MessageBox.Show("YOU CAN'T UPDATE A RECORD RECENT THAN THE DATE ("
                        + DtpCurrDate.Value.ToString("yyyy/MM/dd") + ") SELECTED.");
                }
                else if (SelectedEndDateValue != EndDt.Date)
                {
                    MessageBox.Show("YOU CAN'T UPDATE A RECORD WITH END DATE OTHER THAN \"31-12-2099\"");
                }
                else
                {
                    string PrimaryKeyFields = Check_PrimaryKeyFields();
                    if (PrimaryKeyFields != "")
                    {
                        string StartDateValue = DtpCurrDate.Value.ToString("yyyy/MM/dd");
                        string EndDateValue = Convert.ToDateTime(StartDateValue).AddDays(-1).ToString("yyyy/MM/dd");
                        int OldKeyValue = 0;
                        strQ = "select " + KeyName + " OK from " + TableName
                            + " where " + PrimaryFieldsWhereClauseGlobal + " and EndDate >= '" + EndDateValue + "'";
                        cmd = new SqlCommand(strQ, con);
                        Rdr = cmd.ExecuteReader();
                        if (Rdr.HasRows == true)
                        {
                        ...
                        Rdr.Close();
                        int KeyValue = GetMaxPlusOne();
                        //Update Facts
                        int StartDateKey = mgr.FetchKey("DateDim", DtpCurrDate.Value.ToString("dd-MM-yyyy"));
                        int EndDateKey = 0;
                        //Update existing Dimension record
                        strQ = "update " + TableName + " set EndDate = '" + EndDateValue
                            + "' where " + PrimaryFieldsWhereClauseGlobal + " and EndDate >= '" + EndDateValue + "'";
                        cmd = new SqlCommand(strQ, con);
                        cmd.ExecuteNonQuery();
                        //Insert Record
                        ...
                        {
                            if (Counter == StartDtPos)
                                InsertString = InsertString + "','" + StartDateValue;
                            else
                                InsertString = InsertString + "','" + GetFieldValues(Counter);
                            Counter++;
                        }
